...
GIT_SHA="${VSTL_BUILD_GIT_SHA:-$(git -C "$SCRIPT_DIR" rev-parse --short=12 HEAD 2>/dev/null || true)}"
GIT_BRANCH="${VSTL_BUILD_GIT_BRANCH:-$(git -C "$SCRIPT_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || true)}"
